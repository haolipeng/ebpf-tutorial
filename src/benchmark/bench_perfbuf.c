...
int main(int argc, char **argv)
{
	...
	// 打开并加载 BPF 程序
	skel = bench_perfbuf_bpf__open_and_load();
	if (!skel) {
	...
	// 附加 BPF 程序
	err = bench_perfbuf_bpf__attach(skel);
	if (err) {
		fprintf(stderr, "Failed to attach BPF skeleton\n");
		goto cleanup;
	}

	// 创建 perf buffer (每个 CPU 8 页)
...
cleanup:
	...
	bench_perfbuf_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}
